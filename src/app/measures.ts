    ...
    static load(iMeasures: IMeasures, startIndex: IRangeStart, beatsPerMeasure: number, framework?: string): Measure[] {
        ...
        if (typeof iMeasures === "number")
            for (let i = 0; i < iMeasures; i++)
                measures.push(new Measure(startIndex, beatsPerMeasure, "info", framework));
        else
            iMeasures.forEach((measure) => {
                let length = beatsPerMeasure, context: IContext = "info", warning: boolean = false;
                ...
                context = length !== beatsPerMeasure ? "danger" : warning ? "warning" : "info";
